...
from waybills.serializers import WaybillAuditSerializer, WaybillRevertErrorSerializer
...
def manage_index_view(request):
    qs = Waybill.objects.all().values('status__name', 'status__order_index').annotate(
        cnt=Count('status__name')).order_by('status__order_index')

    # nj 已审核, 其中上传身份证数量
    total_shenhe = Waybill.objects.filter(src_loc__name='NJ').filter(status__name=u'已审核').count()
    total_shenhe_has_id = Waybill.objects.filter(src_loc__name='NJ').filter(status__name=u'已审核').exclude(
        person_id__iexact='').count()
    overview = get_overview(qs)
    overview['NJ 已传ID/已审核'] = '%s/%s' % (total_shenhe_has_id, total_shenhe)
    qs_overview = overview

    local_aware_time = timezone.localtime(timezone.now(), timezone.get_current_timezone())
    date_aware = toTZDatetime(local_aware_time.strftime('%m/%d/%Y'))

    qs_today = WaybillStatus.objects.filter(create_dt__gte=date_aware) \
        .values('status__order_index', 'status__name').annotate(cnt=Count('status__name')).order_by(
        'status__order_index')

    cn_tracking_cnt = QFTracking.objects.filter(is_used=False).count()

    # 总体打包绩效
    qs_group = WaybillStatus.objects.filter(create_dt__gte=date_aware - timezone.timedelta(days=6)).filter(
        status__name='已审核').filter(waybill__src_loc__name__in=['NJ', 'FL']) \
        .extra({'create_dt': 'date(waybills_waybillstatus.create_dt)'}) \
        .values('create_dt', 'status__name') \
        .annotate(cnt=Count('id')) \
        .order_by('create_dt')

    # 按人打包绩效
    qs_group_by_person = WaybillStatus.objects.filter(
        create_dt__gte=date_aware - timezone.timedelta(days=6)).filter(
        status__name='已审核').filter(waybill__src_loc__name__in=['NJ', 'FL']) \
        .extra({'create_dt': 'date(waybills_waybillstatus.create_dt)'}) \
        .values('create_dt', 'status__order_index', 'status__name', 'user__username') \
        .annotate(cnt=Count('id')) \
        .order_by('create_dt', 'user__username', 'status__order_index')

    # 个人当天绩效
    qs_person = WaybillStatus.objects.filter(create_dt__gte=date_aware.date()).filter(status__name__in=['已审核']) \
        .filter(waybill__src_loc__name__in=['NJ', 'FL']) \
    ...
    return render(request, 'manage/index.html',
                  context={"qs": qs_overview, "qs_today": get_today_over_view(qs_today),
                           "qs_group_by_person": qs_group_by_person, "qs_group": qs_group, 'qs_person': qs_person,
                           "isWarehouseManager": is_warehouse_manager(request.user),
                           "isPackageManager": is_package_manager(request.user),
                           "cn_tracking_cnt": cn_tracking_cnt})
...
def performacne_view(request):
    ...
    if form.is_valid():
        ...
        if form.cleaned_data["dt_st"]:
            dt_st = pytz.timezone("America/New_York").localize(
                datetime.strptime(form.cleaned_data["dt_st"], '%m/%d/%Y'))
            q.add(Q(create_dt__gte=dt_st), AND)
        if form.cleaned_data["dt_ed"]:
            dt_ed = pytz.timezone("America/New_York").localize(
                datetime.strptime(form.cleaned_data["dt_ed"], '%m/%d/%Y'))
        ...
        qs_chudan = WaybillStatus.objects.filter(q).extra(
            {'create_dt': 'date(waybills_waybillstatus.create_dt)'}).values('create_dt', 'user__username').annotate(
            cnt=Count('status__name')).order_by('create_dt')
...
def table_helper_chudan(qs):
    ...
    rs = 0
    for o in qs:
        if o['create_dt'] != last_dt:
            result.append({'create_dt': o['create_dt'],
...
def manage_id_card_image_view(request):
    ...
    if request.method == 'GET':
        people = People.objects.filter(Q(status=1), ~Q(id_card_backside__isnull=True),
                                       ~Q(id_card_backside__exact='')).first()

        return render(request, 'manage/waybill_review_id_image.html', {'people': people})
...
def manage_waybills_bulk_print_view(request):
    ...
    total_waybill_cnt, total_a1, total_k2, total_q = 0, 0, 0, 0
    if form.is_valid():
        ...
        total_a1, total_k2, total_q, total_waybill_cnt = cal_total_cnt(dt, qs, src_loc)
    ...
    return render(request, 'manage/waybill_print_bulk_print.html',
                  {'table': table, 'waybill_cnt': waybill_cnt, 'goods_cnt': goods_cnt, 'form': form, 'form_action': '',
                   ...
                   'total_k2': total_k2, 'total_q': total_q})
...
def cal_total_cnt(dt, qs, src_loc):
    total_qs = qs.filter(
        Q(src_loc=src_loc), Q(channel__name__in=['A1', 'K2', 'Q']),
    ...
    return total_a1, total_k2, total_q, total_waybill_cnt
