...
def get_secect(channel_name):
    if settings.DEBUG:
        return 'KkPHQixh7wE2MeyRrAFv8qf4aScmJ3'
    else:
        if channel_name == 'A1':
            return 'nIKAps9N4F8Zuk6SDXVQtJfiMwGecY'
        elif channel_name == 'A2':  # cd
            return 'GZatAWgjewFvBTYNdxuS7RJQ24cyXK'
        elif channel_name == 'A3':  # wh
            return 'wN89U3jISukKAbvT6JPXtMQ5hWDezC'
        else:
            return ''


def get_partnerid(channel_name):
    if settings.DEBUG:
        return '7864641052'
    else:
        if channel_name == 'A1':
...
def upsert_one(w, is_update=False):
    ...
    validation = md5(base64_encode(str(xml)) + get_partnerid(w.channel.name) + get_secect(w.channel.name))
    post_data = {
        'partnerid': get_partnerid(w.channel.name),
        'version': version,
...
def one_by_one_create(qs, channel_name):
    succ_list = []
    fail_list = []
    for w in qs:
        flag = 5
        ...
        while not w.cn_tracking and flag > 0:
            ...
            w.save()
            request_result, succ_cnt, fails = bulk_create([w], channel_name)
...
def bulk_create(ws, channel_name, is_update=False):
    url = update_url if is_update else create_url
    request_result = "请求成功"
    results = []
    succ_cnt = 0
    ...
    validation = md5(base64_encode(str(xml)) + get_partnerid(channel_name) + get_secect(channel_name))
    post_data = {'partnerid': get_partnerid(channel_name),
                 'version': version,
                 'xmldata': base64_encode(str(xml)),
                 'validation': validation,
                 'request': data
        ...
        for r in results:
            if r['status'] == '1':
                ...
                CnTrackingCreateLog.objects.create(us_tracking=r['order_serial_no'], cn_tracking=r['mail_no'],
                                                   msg=r['msg'] + "|" + msg, status=r['status'], channel=w.channel)
            else:
                CnTrackingCreateLog.objects.create(us_tracking=r['order_serial_no'], cn_tracking=r['mail_no'],
                                                   msg=r['msg'], status=r['status'], channel=w.channel)
    else:
...
def cancel(ws):
    '''ws 必须为渠道一致的集合'''
    channel_name = ws[0].channel.name
...
def waybill_process(w, mail_no, status, position, position_no, ch):
    if status == '1':
        try:
...
def auto_create():
    ...
    for ch in ['A1', 'A2', 'A3']:
        qs = Waybill.objects.filter(cn_tracking__isnull=True).filter(channel__name=ch).filter(
            status__name__in=['已建单', '已传身份证'])
        g_cnt = int(math.ceil(len(qs) / 200.0))
        f_list = []
        for i in range(g_cnt):
            request_result, succ_cnt, fail_waybills = bulk_create(qs[i * 200: min(len(qs), (i + 1) * 200)], ch)
            f_list += fail_waybills
            cnt += succ_cnt
        succ_list, fail_list = one_by_one_create(f_list, ch)
        cnt += len(succ_list)
